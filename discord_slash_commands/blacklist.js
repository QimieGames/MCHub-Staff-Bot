					...
					case true:
						...
						if (discordUser === undefined) {
							...
							if (discordUser.roles.cache.some(discordUserRoles => staffBotBlacklistedUserRoleID.includes(discordUserRoles.id)) === true) {
								await discordInteractionDetails.editReply({ content: '```The user is already blacklisted from using this staff bot!```', ephemeral: false }).then(() => {

									discordInteractionResultDetails.interactionResult = false, discordInteractionResultDetails.interactionFailedReason = 'The user is already blacklisted from using this staff bot!';
								...
								if (discordBot.guilds.cache.get(guildID).members.cache.get(clientID).permissions.has('ManageRoles', true) === true) {
									await discordUser.roles.add(staffBotBlacklistedUserRoleID).then(async () => {
										await discordInteractionDetails.editReply({ content: '```Successfully blacklisted the user from using this staff bot.```', ephemeral: false }).then(() => {

											discordInteractionResultDetails.interactionResult = true;

										});
									}).catch(async () => {
										await discordInteractionDetails.editReply({ content: '```Error occured while blacklisting the user from using this staff bot!```', ephemeral: false }).then(() => {

											discordInteractionResultDetails.interactionResult = false, discordInteractionResultDetails.interactionFailedReason = 'Error occured while blacklisting the user from using this staff bot!';
